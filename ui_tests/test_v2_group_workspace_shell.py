...
Group workflow member run/cancel, status gates, delete, and search coverage: 0.261.174
"""
...
def test_group_selection_populates_shared_shell_without_personal_data(group_ui, theme, width, height):
    ...
    # The Owner's server-computed document_management offers upload, so the native command renders
    # in group scope while nothing personal is read.
