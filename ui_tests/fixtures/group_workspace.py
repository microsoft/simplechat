...
def settings_management(role, status):
    ...
    The profile and logo need the owner, and a group that is not locked or inactive; downloads,
    retention, activity and stats need the owner or an admin, and the file count the owner. Retention
    is off, so a manager is told so. Like the server's `group_settings_read_only`, only `locked` and
    `inactive` make the profile read-only: a status the server does not recognize does not.
    """
    owner = role == "Owner"
