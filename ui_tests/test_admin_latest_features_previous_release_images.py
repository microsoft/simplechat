...
Version: 0.260.001
Implemented in: 0.241.002; 0.250.028; 0.250.036; 0.260.001
...
preview modal. It also validates that the current-release admin cards render
their screenshots and rollout notes.
"""
...
def test_admin_latest_features_previous_release_images():
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            data_management_card = page.locator("#latest-features-admin-release-260-data-management-card")
            expect(data_management_card).to_be_visible()
            expect(data_management_card.get_by_role("heading", name="Enterprise Data Management: Backup, Restore & Migration")).to_be_visible()
            data_management_card.get_by_role("link", name="Show Details").click()
            expect(data_management_card.get_by_role("link", name="Open Data Management")).to_be_visible()
            expect(data_management_card.locator("img[src*='admin_release_260_data_management.png']")).to_be_visible()

            mcp_card = page.locator("#latest-features-admin-release-260-mcp-platform-card")
            expect(mcp_card).to_be_visible()
            mcp_card.get_by_role("link", name="Show Details").click()
            expect(mcp_card.get_by_role("heading", name="Screenshot and rollout notes")).to_be_visible()
            expect(mcp_card.locator(".latest-feature-thumbnail-trigger")).to_have_count(1)
