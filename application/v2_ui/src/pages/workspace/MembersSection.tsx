// MembersSection.tsx
// The scope-driven Members section shared by the group workspace (M7B) and the public workspace
// (M10A): who belongs to the workspace, their roles, and who is asking to join, managed natively
// through the scope's /membership/... routes.
//
// The group and public Members views are the same view over different routes and vocabulary, so
// this component holds the whole behaviour once and the two wrappers (GroupMembersSection,
// PublicMembersSection) pass a `scope` that names the one client, the labels and copy, the
// assignable roles, whether a "leave" control exists, and the scope's own error codes. Nothing
// here branches on `scope.kind`: a difference is a value on the scope, never a code path, so the
// group's rendered rows, controls and copy are byte-identical to before the extraction.
//
// Every control still comes from the server, with no fallback: the workspace-level operations
// (add, review requests) from the member list's `membership_management` hint, and each row's
// controls from its own `member_actions`. Whether "leave" can appear is a scope capability *and*
// the server's per-member action, never a client role check. Nothing is kept optimistically. A
// write shows the server's own outcome and the list is read again, so a row, a role and the
// offered controls always show what the server holds. A refusal shows the server's reviewed
// message. A write conflict (only the group has one) keeps the state so the same action can be
// tried again. A refusal that means the caller's own standing changed also re-reads the workspace
// context, so the header and every other section follow. Leaving goes back to the scope's list,
// because the workspace is no longer the caller's.
//
...
import {
    MEMBERS_MAX_PAGE, MEMBERS_PAGE_SIZE, MEMBER_SEARCH_MAX_LENGTH,
    MembershipResponseError, fallbackMemberText, isAccessChangedError,
    isAssignableMemberRole, isGroupMemberRole, isTerminalMembershipError, memberDisplayName, membershipErrorCode,
    membershipErrorMessage,
    type AssignableMemberRole, type DirectoryUser, type GroupMember, type GroupMemberRole,
    type GroupMembershipClient, type JoinRequest,
    type JoinRequestList, type MemberCsvRow, type MemberListPage,
} from '../../lib/groupMembership';

/**
 * Everything that differs between a group and a public Members view. A difference is a value here,
 * never a branch in the component: the group scope reproduces the shipped behaviour exactly.
 */
export interface MembersSectionScope {
    /** Which workspace kind this is. Documentation only; the component never branches on it. */
    kind: 'group' | 'public';
    /** The workspace id, used only for the section's remount key. */
    workspaceId: string;
    /** The workspace's display name, shown in the confirmation copy. */
    workspaceName: string;
    /** The one membership client for this scope's /membership routes. Must be stable. */
    client: GroupMembershipClient;
    /** The friendly label for a member role (never leaks a raw "DocumentManager"). */
    roleLabel: (role: string) => string;
    /** The options for the role select: the roles this scope can assign. */
    assignableOptions: readonly { value: AssignableMemberRole; label: string }[];
    /** The bulk role select's initial value; must be one of `assignableOptions`. */
    defaultBulkRole: AssignableMemberRole;
    /** Whether this scope offers "leave" at all. A leave control also needs the server's action. */
    supportsLeave: boolean;
    /** The common noun for the workspace, e.g. "group" or "public workspace". */
    noun: string;
    /** The Members section's intro line. */
    introDescription: string;
    /** The add-member status-unavailable code, which closes the dialog and refreshes. */
    statusUnavailableCode: string;
    /** The write-conflict code that keeps state for a retry, or null when the scope has none. */
    writeConflictCode: string | null;
    /**
     * The role the current owner is left with after transferring ownership. The group demotes the
     * old owner to a plain member ('User'); the public workspace keeps them as a document manager
     * (decision 21), so the "you're now a …" copy names the right role per scope.
     */
    demotedOwnerRole: GroupMemberRole;
    /**
     * The roles offered in the "filter by role" select. The group lists every role including a
     * plain member; the public workspace has no plain members, so it lists only Owner, Admin and
     * Document manager. A role outside this list in the URL is still honoured, just not offered.
     */
    filterRoles: readonly GroupMemberRole[];
}
...
function MemberRow({
    member, isSelf, selectable, selected, pendingRole, disabled, busy, roleLabel, assignableOptions, supportsLeave,
            ...
            <div className="ml-auto flex min-w-0 flex-wrap items-center justify-end gap-2">
                ...
                {canChangeRole ? (
                    <select className={SELECT_CLASS} value={pendingRole ?? member.role} disabled={disabled}
                        ...
                        {assignableOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                    </select>
                ) : <Pill tone={member.role === 'Owner' ? 'accent' : 'neutral'}>{roleLabel(member.role)}</Pill>}
                ...
                {supportsLeave && member.actions.includes('leave') && isSelf ? (
...
export function MembersSection({
    scope, viewerId, interactionDisabled, onBusyChange, onAccessChanged, onLeft,
}: {
    scope: MembersSectionScope;
    viewerId: string;
    ...
    /** Leave the page after the caller left the workspace. Only reached when the scope offers leave. */
    ...
    const [bulkRole, setBulkRole] = useState<AssignableMemberRole>(scope.defaultBulkRole);
    ...
    const changeRole = async (member: GroupMember, role: AssignableMemberRole) => {
        ...
        setNotice({
            tone: 'status',
            text: self ? `Your role is now ${scope.roleLabel(result.value.member.role)}.`
                : `${name}'s role is now ${scope.roleLabel(result.value.member.role)}.`,
        });
        reloadList();
    ...
    const confirmedRefusal = (error: unknown, fallback: string) => {
        if (scope.writeConflictCode !== null && membershipErrorCode(error) === scope.writeConflictCode) {
    ...
    const removeMember = async (member: GroupMember) => {
        ...
        setConfirmation(null);
        setNotice({ tone: 'status', text: `${memberDisplayName(member)} was removed from the ${scope.noun}.` });
        reloadAll();
    };
    ...
    const leaveGroup = async (member: GroupMember) => {
        ...
        if (!result.ok) {
            confirmedRefusal(result.error, `You could not leave the ${scope.noun}. Please retry.`);
            return;
        }
        setConfirmation(null);
        await onLeftRef.current?.();
    };
    ...
    const transferOwnership = async (member: GroupMember) => {
        ...
        if (result.value.changed) {
            setNotice({ tone: 'status', text: `${memberDisplayName(result.value.owner)} is now the owner. You're now a ${scope.roleLabel(scope.demotedOwnerRole)}.` });
        }
        reloadAll();
        ...
        if (result.ok) {
            setNotice({
                ...
                text: result.value.alreadyMember ? `${name} was already a member. Their request to join was cleared.`
                    : `${name} was added to the ${scope.noun}.`,
            });
            reloadAll();
            return;
        }
    ...
    const addMember = async (user: DirectoryUser, role: AssignableMemberRole) => {
        ...
        if (result.ok) {
            setAddOpen(false);
            setNotice({ tone: 'status', text: `${memberDisplayName(result.value)} was added as ${scope.roleLabel(result.value.role)}.` });
            reloadAll();
            return;
        }
        ...
        if (code === scope.statusUnavailableCode || isAccessChangedError(result.error)) {
            setAddOpen(false);
    ...
    const runBulk = async (kind: 'role' | 'remove', targets: GroupMember[], role: AssignableMemberRole) => {
        ...
        const summary = kind === 'role'
            ? [
                `Changed ${plural(changed, 'member', 'members')} to ${scope.roleLabel(role)}.`,
    ...
    const confirmationDialog = (() => {
        ...
        if (confirmation.kind === 'remove') {
            ...
            return (
                <ConfirmDialog title={`Remove ${name}?`} confirmLabel="Remove member" confirmIcon={<UserMinus size={14} />}
                    description={`${name} will lose access to ${scope.workspaceName} and everything shared in it.`}
        ...
        if (confirmation.kind === 'leave') {
            return (
                <ConfirmDialog title={`Leave ${scope.workspaceName}?`} confirmLabel="Leave group" confirmIcon={<LogOut size={14} />}
                    description={`You'll lose access to this ${scope.noun} and everything shared in it.`}
                    busy={busy} onClose={close} onConfirm={() => void leaveGroup(confirmation.member)}>
                    <div className="space-y-2">
                        <p className="text-xs text-text-2">To come back, ask an owner or admin, or request to join from the {scope.noun} directory.</p>
                        {errorLine}
                    </div>
                </ConfirmDialog>
            );
                ...
                <ConfirmDialog title={`Make ${name} the owner?`} confirmLabel="Transfer ownership" confirmIcon={<Crown size={14} />}
                    ...
                    <div className="space-y-2">
                        <ul className="list-disc space-y-1 pl-5 text-xs text-text-2">
                            <li className="break-words">{name} becomes the owner of {scope.workspaceName}.</li>
                            <li>You become a {scope.roleLabel(scope.demotedOwnerRole)}. You lose the owner's abilities, including managing members and transferring ownership, unless the new owner gives you a role.</li>
        ...
        if (confirmation.kind === 'demote') {
            const label = scope.roleLabel(confirmation.role);
            return (
                <ConfirmDialog title="Change your own role?" confirmLabel="Change my role"
                    ...
                    <p className="text-xs text-text-2">You'll no longer be able to manage this {scope.noun}'s members. Only the owner or another admin can make you an admin again.</p>
                </ConfirmDialog>
            );
        }
        ...
        return (
            <ConfirmDialog title={`Remove ${plural(targets.length, 'member', 'members')}?`}
                ...
                description={`They'll lose access to ${scope.workspaceName} and everything shared in it.`}
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <SectionIntro title="Members" description={scope.introDescription} />
                {canAdd ? (
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <select className={clsx(SELECT_CLASS, 'py-2')} aria-label="Filter by role" value={urlRole ?? ''}
                    ...
                    {scope.filterRoles.map((role) => <option key={role} value={role}>{scope.roleLabel(role)}</option>)}
                </select>
            </div>
                    ...
                    {bulkRoleTargets.length ? (
                        <>
                            <select className={SELECT_CLASS} aria-label="New role for the selected members" value={bulkRole}
                                ...
                                {scope.assignableOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                            </select>
            ...
            {!list && loading ? (
                ...
                <EmptyState icon={<Users size={28} />} title="No members to show"
                    description={urlSearch || urlRole ? 'No members match your search.' : `This ${scope.noun} has no members yet.`} />
            ) : (
                <>
                            ...
                            return (
                                <li key={member.userId}>
                                    <MemberRow member={member} isSelf={isSelf}
                                        ...
                                        roleLabel={scope.roleLabel} assignableOptions={scope.assignableOptions} supportsLeave={scope.supportsLeave}
            ...
            {addOpen ? (
                <AddMemberDialog submitting={busyKey === 'add'} serverError={addError}
                    roleOptions={scope.assignableOptions} defaultRole={scope.defaultBulkRole}
