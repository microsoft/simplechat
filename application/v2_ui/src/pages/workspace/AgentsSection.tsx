...
export function AgentsSection({ actionsEnabled, adapter = PERSONAL_AGENT_WORKBENCH }: { actionsEnabled: boolean; adapter?: AgentWorkbenchAdapter }) {
    ...
    const { items, loading, error, refresh, setItems, setError } =
        useSectionResource<AgentConfiguration>(adapter.listAgents, 'Failed to load agents.');
    ...
    const remove = async (agent: AgentConfiguration) => {
        if (isGroup ? !adapter.allows('delete', agent) : agent.is_global) return;
        setBusyId(agent.id);
        setError(null);
        try {
            await adapter.deleteAgent(agent);
        ...
        <div className="flex h-full min-h-0 flex-col gap-4">
            ...
            <div className="shrink-0 space-y-4">
                <SectionIntro title="Agents"
                    description={isGroup
                        ? 'Build reusable assistants for this group, with their own model, instructions, knowledge, and actions. Provided agents can be viewed and used, but not changed here.'
                        : 'Build reusable assistants with their own model, instructions, knowledge, and actions. Provided agents can be viewed and used, but not changed here.'}
                    actions={<GlassButton type="button" variant="primary" size="sm" disabled={isGroup && !adapter.allows('create')}
                        onClick={() => navigate(`${adapter.basePath}/new`)}><Plus size={14} />New agent</GlassButton>} />
                <div className="flex flex-wrap items-center gap-2">
                    <GlassButton type="button" size="sm" onClick={() => navigate(`${adapter.basePath}/new?templates=1`)}><Sparkles size={14} />Examples &amp; templates</GlassButton>
                ...
                <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_10rem_9rem]">
                    ...
                    <select aria-label="Filter agent scope" value={filters.scope} className={AGENT_INPUT_CLASS} onChange={(event) => setFilters((current) => ({ ...current, scope: event.target.value }))}>
                        <option value="">All ownership</option><option value="personal">{isGroup ? 'Group' : 'Personal'}</option><option value="provided">Provided</option>
                    </select>
                </div>
            ...
            <div ref={list} className="min-h-0 flex-1 overflow-y-auto pb-4 pr-1"
                ...
                <ul className={filters.view === 'cards' ? 'grid gap-3 lg:grid-cols-2 2xl:grid-cols-3' : 'space-y-3'}>
                    {visible.map((agent) => {
                        ...
                        const editable = isGroup ? adapter.allows('edit', agent) : !provided;
                        const deletable = isGroup ? adapter.allows('delete', agent) : !provided;
                                ...
                                <GlassPanel elevation="flat" className={`h-full p-4 ${filters.view === 'cards' ? 'space-y-3' : 'flex flex-wrap items-start gap-3'}`}>
                                    <div className="flex min-w-0 flex-1 items-start gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="mt-1 break-all text-[10px] text-text-3">{provided ? 'global' : isGroup ? 'group' : 'personal'} · {agent.id}</p>
                                            {agent.tags?.length ? <p className="mt-1 break-words text-xs text-text-3">{agent.tags.join(' · ')}</p> : null}
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        <GlassButton type="button" size="sm" onClick={() => navigate(path)}>{editable ? 'Edit' : 'View details'}</GlassButton>
                                        {adapter.canUseInChat(agent) ? <Link to={chatHrefForAgent(agent.id, adapter.chatScope(agent))} className="rounded-lg px-2 py-1.5 text-xs font-medium text-accent hover:bg-accent-soft">Use in chat</Link> : null}
                                        {deletable ? <ConfirmAction icon={<Trash2 size={15} />} label={`Delete ${label}`} confirmLabel="Delete agent"
