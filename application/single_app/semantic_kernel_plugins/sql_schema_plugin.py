...
class SQLSchemaPlugin(BasePlugin):
    ...
    def _create_connection(self):
        ...
        try:
            ...
            log_event(f"[SQLSchemaPlugin] Connection failed", extra={"database_type": self.database_type, "error": str(e)})
            raise
        ...
        try:
            ...
            for table in tables:
                ...
                try:
                    ...
                    # Log per-table errors with full detail for admin — not exposed to caller
                    debug_print(f"[SQLSchemaPlugin] Error getting schema for table {qualified_table_name}: {str(table_err)}")
            ...
            try:
                ...
                debug_print(f"[SQLSchemaPlugin] Error getting relationships: {str(rel_err)}")
                log_event(f"[SQLSchemaPlugin] Error getting relationships", extra={"error": str(rel_err)})
            ...
            # Log full exception detail for admin/App Insights — NEVER expose str(e) to user
            # str(e) can contain hostnames, DB names, table names, connection info, SQL fragments
            debug_print(f"[SQLSchemaPlugin] get_database_schema failed: {str(e)}")
            log_event(f"[SQLSchemaPlugin] get_database_schema failed", extra={
                "error": str(e),
                "database_type": self.database_type,
                "database": self.database
            })
            # Sanitized generic message returned to user
            return ResultWithMetadata(
                {"error": "An error occurred while retrieving the database schema. Please contact your administrator."},
    ...
    def get_table_schema(self, table_name: str) -> ResultWithMetadata:
        ...
        try:
            ...
            # Log full detail for admin — sanitized message returned to caller
            debug_print(f"[SQLSchemaPlugin] Error getting table schema for {table_name}: {str(e)}")
            log_event(f"[SQLSchemaPlugin] get_table_schema failed", extra={
                "table_name": table_name,
                "error": str(e)
            })
            return ResultWithMetadata(
                {"error": "An error occurred while retrieving the table schema. Please contact your administrator."},
                {"source": "sql_schema_plugin", "success": False}
            )
    ...
    def get_table_list(
        ...
        try:
            ...
            # Log full detail for admin — sanitized message returned to caller
            debug_print(f"[SQLSchemaPlugin] Error getting table list: {str(e)}")
            log_event(f"[SQLSchemaPlugin] get_table_list failed", extra={"error": str(e)})
            return ResultWithMetadata(
                {"error": "An error occurred while retrieving the table list. Please contact your administrator."},
                {"source": "sql_schema_plugin", "success": False}
            )
    ...
    def get_relationships(self, table_name: Optional[str] = None) -> ResultWithMetadata:
        ...
        try:
            ...
            # Log full detail for admin — sanitized message returned to caller
            debug_print(f"[SQLSchemaPlugin] Error getting relationships: {str(e)}")
            log_event(f"[SQLSchemaPlugin] get_relationships failed", extra={"error": str(e)})
            return ResultWithMetadata(
                {"error": "An error occurred while retrieving relationships. Please contact your administrator."},
                {"source": "sql_schema_plugin", "success": False}
            )
    ...
    def _get_relationships_data(self, cursor, table_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            debug_print(f"[SQLSchemaPlugin] Error executing relationships query: {str(e)}")
            log_event(f"[SQLSchemaPlugin] Error getting relationships", extra={"error": str(e)})
