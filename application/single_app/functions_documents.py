        ...
        for retry in range(max_retries + 1):  # +1 for the initial attempt
            try:
                ...
                if retryable:
                    ...
                    # Final attempt: If this is the last retry for a 403 Forbidden error,
                    # try a fallback authentication method
                    if retry == max_retries - 1 and status_code == 403:
                        try:
                            print("Attempting fallback authentication as last resort...")
                            settings = get_settings()
                            ...
                            add_file_task_to_file_processing_log(
                                document_id=document_id,
                                user_id=group_id if is_group else user_id,
                                content=f"Attempting fallback authentication after repeated Forbidden errors"
                            )
                            
                            # If using APIM, try direct connection
                            if settings.get('enable_ai_search_apim'):
                                from azure.search.documents import SearchClient
                                from azure.core.credentials import AzureKeyCredential
                                
                                print("Trying direct connection instead of APIM...")
                                fallback_client = SearchClient(
                                    endpoint=settings.get('azure_ai_search_endpoint'),
                                    index_name=search_client._index_name,
                                    credential=AzureKeyCredential(settings.get('azure_ai_search_key'))
                                )
                                fallback_client.upload_documents(documents=[chunk_document])
                                print("Fallback authentication succeeded!")
                                
                                # Log success
                                add_file_task_to_file_processing_log(
                                    document_id=document_id,
                                    user_id=group_id if is_group else user_id,
                                    content=f"Fallback authentication succeeded using direct connection"
                                )
                                return  # Exit the function successfully
                            
                            # If using key auth, try managed identity
                            elif settings.get('azure_ai_search_authentication_type') != 'managed_identity':
                                from azure.search.documents import SearchClient
                                from azure.identity import DefaultAzureCredential
                                
                                print("Trying managed identity instead of API key...")
                                fallback_client = SearchClient(
                                ...
                                add_file_task_to_file_processing_log(
                                    document_id=document_id,
                                    user_id=group_id if is_group else user_id,
                                    content=f"Fallback authentication succeeded using managed identity"
                                )
                                return  # Exit the function successfully
                            
                        except Exception as fallback_err:
                            print(f"Fallback authentication failed: {repr(fallback_err)}")
                            # Log fallback failure
                            add_file_task_to_file_processing_log(
                                document_id=document_id,
                                user_id=group_id if is_group else user_id,
                                content=f"Fallback authentication failed: {repr(fallback_err)}"
                            )
                            # Continue with the normal retry process
