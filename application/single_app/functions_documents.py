...
from functions_dlp import (
    build_dlp_review_event_summary,
    ...
    try:
        ...
        try:
            ...
            if is_public_workspace:
                ...
                meta = get_document_metadata(document_id=document_id, user_id=user_id)
...
def process_video_document(
    ...
    try:
        ...
        sanitized_error = _sanitize_video_indexer_log_value(e)
        debug_print(f"[VIDEO INDEXER] Authentication failed: {sanitized_error}")
        print(f"[VIDEO] AUTH ERROR: {sanitized_error}", flush=True)
        update_callback(status=f"VIDEO: auth failed → {sanitized_error}")
        return 0
    ...
    try:
        ...
        debug_print(
            "[VIDEO INDEXER] Upload params keys: "
            f"{list(params.keys())}; "
            f"name_length={len(original_filename or '')}; "
            f"indexingPreset={params.get('indexingPreset')}; "
            f"streamingPreset={params.get('streamingPreset')}; "
            f"accessToken_present={bool(token)}"
        )
        ...
        sanitized_error = _sanitize_video_indexer_log_value(e)
        debug_print(f"[VIDEO INDEXER] Upload request failed: {sanitized_error}")
        ...
        print(f"[VIDEO] UPLOAD ERROR: {sanitized_error}", flush=True)
        update_callback(status=f"VIDEO: upload failed → {sanitized_error}")
        return 0
    except Exception as e:
        sanitized_error = _sanitize_video_indexer_log_value(e)
        debug_print(f"[VIDEO INDEXER] Upload unexpected error: {sanitized_error}")
        print(f"[VIDEO] UPLOAD ERROR: {sanitized_error}", flush=True)
        update_callback(status=f"VIDEO: upload failed → {sanitized_error}")
        return 0
    ...
    debug_print(
        "[VIDEO INDEXER] Index polling request prepared: "
        f"endpoint={vi_ep}/{vi_loc}/Accounts/{vi_acc}/Videos/[video-id]/Index; "
        f"video_id_length={len(str(vid or ''))}; "
        f"accessToken_present={bool(token)}"
    )
    ...
    while True:
        ...
        try:
            ...
            sanitized_error = _sanitize_video_indexer_log_value(e)
            debug_print(f"[VIDEO INDEXER] Poll request failed: {sanitized_error}")
            if hasattr(e, 'response') and e.response is not None:
            ...
            debug_print(f"[VIDEO INDEXER] Poll unexpected error: {_sanitize_video_indexer_log_value(e)}")
            if poll_count >= max_polls:
                update_callback(status="VIDEO: polling timeout")
                return 0
            time.sleep(30)
    ...
    for chunk_num in range(num_chunks):
        ...
        if chunk_text:
            ...
            debug_print(f"[VIDEO INDEXER] Chunk {chunk_num + 1} has no speech, using insight summary text length: {len(chunk_text)}")
...
def save_chunks(page_text_content, page_number, file_name, user_id, document_id, group_id=None, public_workspace_id=None):
    ...
    try:
        ...
        add_file_task_to_file_processing_log(
            document_id=document_id, 
            user_id=public_workspace_id if is_public_workspace else (group_id if is_group else user_id), 
            content=f"Saving chunk, cosmos_container:{cosmos_container}, page_number:{page_number}, text_length:{len(page_text_content or '')}, user_id:{user_id}, document_id:{document_id}, group_id:{group_id}, public_workspace_id:{public_workspace_id}"
        )
...
def save_chunks_batch(chunks_data, user_id, document_id, group_id=None, public_workspace_id=None):
    ...
    for chunk_info in chunks_data:
        ...
        original_chunk_text = chunk_info['page_text_content']
        enhanced_chunk_text = original_chunk_text + vision_text if vision_text else original_chunk_text
        upload_dlp_result = _evaluate_upload_dlp_text(
            enhanced_chunk_text,
            user_id=user_id,
            document_id=document_id,
...
def extract_document_metadata(document_id, user_id, group_id=None, public_workspace_id=None):
    ...
    try:
        ...
        add_file_task_to_file_processing_log(
            ...
            content=f"Retrieved {len(document_items)} document item(s) for document {document_id}"
        )
    except Exception as e:
        add_file_task_to_file_processing_log(
            document_id=document_id, 
    ...
    add_file_task_to_file_processing_log(
        document_id=document_id, 
        user_id=group_id if is_group else user_id,
        content=f"GPT response for document {document_id} received, length: {len(response_content or '')}"
    )
    ...
    try:
        ...
        add_file_task_to_file_processing_log(
            document_id=document_id, 
            user_id=group_id if is_group else user_id, 
            content=f"Cleaned JSON from GPT response for document {document_id}, length: {len(cleaned_str or '')}"
        )
        ...
        add_file_task_to_file_processing_log(
            document_id=document_id, 
            user_id=group_id if is_group else user_id,
            content=f"Decoded JSON from GPT response for document {document_id}, fields: {sorted(gpt_output.keys()) if isinstance(gpt_output, dict) else []}"
        )
...
def process_audio_document(
    ...
    if AZURE_ENVIRONMENT in ("usgovernment", "custom"):
        for idx, chunk_path in enumerate(chunk_paths, start=1):
            ...
            def recognized_cb(evt):
                try:
                    if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
                        ...
                        print(f"[Debug] Recognized speech segment length: {len(evt.result.text)}")
            ...
            #     print(f"[Debug] Recognized speech segment length: {len(result.text)}")
