# functions_public_document_publication.py
...
def _has_publication(document):
    # Single source of truth lives in the lightweight policy layer so the
    # read/list path and this adapter cannot drift. See
    # functions_public_document_policy.public_document_has_publication.
...
def decide_public_document_publication(user_id, public_workspace_id, document_id, action, payload):
    ...
    from functions_public_document_collaboration import (
        ...
        PublicDocumentCollaborationError,
        _check_etag,
        _checkpoint,
        _context,
        _operation,
    ...
    from functions_public_document_management import require_payload, validate_public_document_id
    ...
    if old_choice is not None and old_choice != choice:
        raise PublicDocumentCollaborationError("decision_conflict", "A different publication decision has already committed.", 409)
    if action == "cancel_artifact" and receipt.get("actor_user_id") != user_id:
        raise PublicDocumentCollaborationError("requester_required", "Only the actual requester can cancel this publication.", 403)
    if action != "cancel_artifact" and role not in PUBLIC_DOCUMENT_MANAGER_ROLES:
    ...
    operation = {
        ...
        "document_version": _version(document), "source_public_workspace_id": public_workspace_id,
        "actor_public_workspace_id": public_workspace_id, "actor_user_id": user_id, "action": action,
    ...
    def guard():
        current_workspace, current_role = require_public_document_read_context(user_id, public_workspace_id)
        operations = public_document_collaboration_operations(current_workspace, current_role, get_settings())
        if action not in operations:
            raise PermissionError("The hosting workspace no longer permits this decision.")
        if not deleted:
            current, current_operation = _operation_document(document_id, operation_id)
            require_execution(current_operation)
            if current["public_workspace_id"] != public_workspace_id or _version(current) != expected_version:
                raise ValueError("The hosting revision changed.")
            if choice == "approved" and not is_current_public_document(current):
                raise ValueError("The hosting document became historical.")
    ...
    def cleanup(current):
        ...
        guard()
        if (
            current.get("public_workspace_id") != public_workspace_id or current.get("id") != document_id
            or _version(current) != expected_version
        ):
            raise ValueError("The publication cleanup target changed.")
        family = public_document_family_records(current)
        ...
        def cleanup_guard(document_id=None):
            if document_id is None:
                current_workspace, current_role = require_public_document_read_context(user_id, public_workspace_id)
                if action not in public_document_collaboration_operations(current_workspace, current_role, get_settings()):
                    raise PermissionError("The hosting workspace no longer permits cleanup.")
                return
            guard()
            ...
            latest = read_public_document_record(current["id"])
            if is_current_public_document(latest) != was_current:
                raise ValueError("The publication cleanup revision selection changed.")

        with public_collaboration_projection_context(document_id, operation_id, execution_token):
            delete_document_revision(
                user_id, current["id"], public_workspace_id=public_workspace_id, delete_mode="current_only",
    ...
    if choice == "approved":
        current = read_public_document_record(document_id)
        ...
        try:
            read_public_document_record(document_id)
    ...
    if not errors:
        try:
            ...
            delete_notifications_by_metadata(
                metadata_filters={
                    "public_workspace_id": public_workspace_id, "document_id": document_id,
                    "publication_receipt_id": receipt["id"], "request_type": "generated_artifact_promotion",
                },
    ...
    return _operation_receipt(public_workspace_id, document, action, status, state, errors), (
