...
_mcp = FastMCP("simplechat-mcp-server", host=MCP_BIND_HOST, port=MCP_BIND_PORT)
...
def show_user_profile(ctx: Context[Any, Any, Any]) -> Dict[str, Any]:
    ...
    # Extract app roles and delegated scopes separately.
    app_roles = claims.get("roles", [])
    if not isinstance(app_roles, list):
        app_roles = [app_roles] if app_roles else []
    scp_raw = claims.get("scp", "")
    scopes = scp_raw.split() if isinstance(scp_raw, str) and scp_raw.strip() else []
...
_TOOL_HELP: Dict[str, Dict[str, Any]] = {
    "help": {
        ...
        "examples": [
            'help',
            'help tool_name="list_personal_documents"',
        ],
    },
    "login_via_oauth": {
        "category": "Authentication",
        "description": (
            "Start device-code OAuth login. Returns a user_code and verification_uri. "
            "Open the URI in a browser and enter the code to authenticate. "
            "The MCP server automatically creates a SimpleChat session after sign-in completes."
        ),
        "auth_required": False,
        "simplechat_api": None,
        "parameters": {},
        "examples": ["login_via_oauth"],
        "notes": "Only needed when not using PRM (bearer token) authentication.",
    },
    "oauth_login_status": {
        "category": "Authentication",
        "description": (
            "Return current login status for both PRM (bearer token) and device-code flows. "
            "Reports whether the user is logged in and which auth method is active."
        ),
        "auth_required": False,
        "simplechat_api": None,
        "parameters": {},
        "examples": ["oauth_login_status"],
    },
    "show_user_profile": {
        "category": "User Profile",
        "description": (
            "Return the authenticated user's SimpleChat profile including userId, displayName, "
            "email, UPN, roles, and all token claims."
        ),
        "auth_required": True,
        "simplechat_api": "POST /external/login",
        "parameters": {},
        "examples": ["show_user_profile"],
    },
    "list_personal_documents": {
        "category": "Personal Workspace",
        "description": "Return the authenticated user's personal workspace documents with pagination and filtering.",
        "auth_required": True,
        "simplechat_api": "GET /api/documents",
        "parameters": {
            "page": "(optional, default 1) Page number.",
            "page_size": "(optional, default 10) Items per page.",
            "search": "(optional) Search by file name or title (case-insensitive substring).",
            "classification": '(optional) Filter by classification. Use "none" for unclassified.',
            "author": "(optional) Filter by author name (substring).",
            "keywords": "(optional) Filter by keyword (substring).",
        },
        "examples": [
            'list_personal_documents',
            'list_personal_documents page=2 page_size=25',
            'list_personal_documents search="project plan" classification="internal"',
        ],
    },
    "list_personal_prompts": {
        "category": "Personal Workspace",
        "description": "Return the authenticated user's personal prompts with pagination and search.",
        "auth_required": True,
        "simplechat_api": "GET /api/prompts",
        "parameters": {
            "page": "(optional, default 1) Page number.",
            "page_size": "(optional, default 10) Items per page.",
            "search": "(optional) Search by prompt name (case-insensitive substring).",
        },
        "examples": [
            'list_personal_prompts',
            'list_personal_prompts search="summarize"',
        ],
    },
    "search_personal_documents": {
        "category": "Search",
        "description": (
            "Hybrid (text + vector) search across the user's personal documents only. "
            "Group and public workspace documents are never searched."
        ),
        "auth_required": True,
        "simplechat_api": "POST /api/search (doc_scope=personal)",
        "parameters": {
            "query": "(required) The search query text.",
            "document_id": "(optional) Restrict search to a specific document ID.",
            "top_n": "(optional, default 12) Maximum number of results.",
        },
        "examples": [
            'search_personal_documents query="project timeline"',
            'search_personal_documents query="budget" top_n=5',
        ],
    },
}
...
def help_tool(tool_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if tool_name and tool_name.strip():
        ...
        # If the tool is unapproved and unapproved tools are disabled, treat as unknown.
        if info and info.get("unapproved") and not _ENABLE_UNAPPROVED_TOOLS:
            info = None
        if not info:
            return {
                "error": "unknown_tool",
                ...
                "available_tools": sorted(
                    n for n, i in _TOOL_HELP.items()
                    if not (i.get("unapproved") and not _ENABLE_UNAPPROVED_TOOLS)
                ),
            }
        result: Dict[str, Any] = {
            "tool_name": name,
            ...
            "simplechat_api": info.get("simplechat_api"),
    ...
    for name, info in _TOOL_HELP.items():
        is_unapproved = info.get("unapproved", False)
        # Completely hide unapproved tools when ENABLE_UNAPPROVED_TOOLS is off.
        if is_unapproved and not _ENABLE_UNAPPROVED_TOOLS:
            continue
