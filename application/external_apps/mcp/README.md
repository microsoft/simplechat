...
This MCP server provides **11 active tools** for interacting with SimpleChat via the Model Context Protocol (Streamable HTTP transport).
...
|------|------|-------------|
| **help** | None | Lists all available tools with descriptions, or returns detailed usage for a specific tool. Params: `tool_name` (optional). |
...
- The token includes the **ExternalApi** or **McpServerAccess** app role
...
- Run locally directly: `python mcp_server.py`
...
- `OAUTH_SCOPES` — Space-separated scopes (e.g. `api://<CLIENT_ID>/.default`)
