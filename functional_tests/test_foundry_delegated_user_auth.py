...
def test_api_key_endpoint_auth_for_foundry_applications_and_workflows() -> None:
    ...
    runtime_content = read_text(RUNTIME_FILE)
    if 'headers["Authorization"] = f"Bearer {credential.api_key}"' in runtime_content:
        raise AssertionError("API-key Foundry calls must not send an Authorization bearer header.")
...
def test_manual_api_key_modal_and_agent_secret_storage() -> None:
    """Validate manual Foundry app/workflow API keys can be entered and protected."""
    print("Testing manual Foundry application/workflow API-key configuration...")

    modal_html_snippets = [
        'id="agent-foundry-api-key-group"',
        'for="agent-foundry-api-key"',
        'id="agent-foundry-api-key"',
        'type="password"',
        'autocomplete="new-password"',
        'data-bwignore="true"',
        'Saved connections use their stored key automatically.',
    ]
    ...
    modal_js_snippets = [
        ...
        "foundryApiKeyGroup.classList.toggle('d-none', !(isNewFoundry || isFoundryWorkflow))",
        ...
        "New Foundry applications can use the signed-in user or a project API key.",
        "Foundry workflows can use the signed-in user or a project API key.",
    ]
    ...
    for snippet in keyvault_snippets:
        assert_contains(KEYVAULT_FILE, snippet)
    ...
    print("✅ Manual Foundry application/workflow API-key configuration verified.")
...
def test_modal_uses_delegated_user_auth_and_safe_consent_link() -> None:
    ...
    modal_js_snippets = [
        ...
        "New Foundry applications can use the signed-in user or a project API key. Actions are disabled.",
        "Foundry workflows can use the signed-in user or a project API key. Actions are disabled.",
    ]
    for snippet in modal_js_snippets:
        assert_contains(MODAL_JS_FILE, snippet)
...
def run_tests() -> bool:
    tests = [
        ...
        test_manual_api_key_modal_and_agent_secret_storage,
