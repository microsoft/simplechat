# test_public_identity_fixture_parity.py
"""
Per-route shape parity between the M10B public identity UI fixture and the real routes.
Version: 0.261.179
Implemented in: 0.261.179

The V2 public Identities section and its reusable-identity picker mock the network with the closed
HTTP fixture ``ui_tests/fixtures/public_identities.py``. A fixture whose response shape drifts from
the server would let a passing browser suite hide a real regression -- the M5B F1/F2 class, where a
fixture invented keys the server never sends. This pin ties the public fixture to the real immutable
``/api/public-workspaces/<workspace_id>/identities[...]`` routes.
...
the still-in-use delete refusal -- it asserts the fixture never invents a top-level, item,
credentials or reference key the server omits (``fixture keys <= server keys``), that the keys the UI
reads are present in both, and that the status code and machine-readable ``error_code`` match,
including its absence. The item scope key is ``public_workspace_id``, the field the public reader
validates every returned identity against, exactly as the group family validates ``group_id``.

The real routes run through the isolated public identity backend harness
(``test_support/public_identity_harness.py``: the real policy, access, projection, storage and route
...
from ui_tests.fixtures.public_identities import (
...
# The fixture's manager workspace. Its ids differ from the harness ids on purpose (the two model the
# same shape independently); each side uses its own list path and etag lookup.
FIXTURE_WORKSPACE = "pub-a"
FIXTURE_MEMBER_WORKSPACE = "pub-b"
FIXTURE_LIST_PATH = f"/api/public-workspaces/{FIXTURE_WORKSPACE}/identities"
...
# id/public_workspace_id (scope assertion), etag (conditional write), identity_actions (per-row gate),
# and the name/description/usage_contexts/credentials the editor draft reads (`draftFromIdentity`).
IDENTITY_ITEM_UI_KEYS = {
    "id", "public_workspace_id", "etag", "identity_actions",
...
def new_fixture():
    return PublicIdentitiesFixture(_FakePage())


def fixture_item_path(identity_id=EDITABLE_IDENTITY_ID):
    return f"{FIXTURE_LIST_PATH}/{identity_id}"
...
# --------------------------------------------------------------------------
# Parity assertions (top-level and nested), mirroring the group identity parity test.
# --------------------------------------------------------------------------
...
def test_update_shape_parity(environment):
    ...
    status, payload = drive_fixture(fixture, "PATCH", fixture_item_path(), body={
        ...
        "expected_etag": fixture._identity_etag(FIXTURE_WORKSPACE, EDITABLE_IDENTITY_ID),
    })
...
def test_identity_in_use_delete_shape_parity(environment):
    ...
    status, payload = drive_fixture(fixture, "DELETE", fixture_item_path(IN_USE_IDENTITY_ID), body={
        "expected_etag": fixture._identity_etag(FIXTURE_WORKSPACE, IN_USE_IDENTITY_ID),
    })
...
def test_role_refusal_403_shape_parity(environment):
    ...
    ordinary user of the public workspace is refused; the fixture models the same refusal on its
    member workspace (pub-b), so the reader never falls back to a personal identity read."""
    seed_identity(environment)
    as_user(environment, "stranger")
    real = environment.client.get(LIST_PATH)
    status, payload = drive_fixture(
        new_fixture(), "GET", f"/api/public-workspaces/{FIXTURE_MEMBER_WORKSPACE}/identities")
...
def test_unexpected_query_400_shape_parity(environment):
    ...
    status, payload = drive_fixture(new_fixture(), "GET", FIXTURE_LIST_PATH, query={"page": ["2"]})
