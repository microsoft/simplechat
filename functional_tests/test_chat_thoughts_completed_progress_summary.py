...
def test_completed_thought_progress_summaries_are_not_live() -> None:
    ...
    review_content = _read('application/single_app/functions_exhaustive_document_review.py')
    ...
    assert 'VERSION = "0.241.099"' in config_content, (
        'Expected config.py version 0.241.099 for the completed thought progress summary fix.'
    )
    assert 'const isCompleted = state.completed || (counters.totalCount > 0 && counters.runningCount === 0);' in thoughts_content, (
    ...
    assert "'type': 'reduction_completed'" in review_content, (
        'Expected exhaustive review progress updates to emit a terminal reduction completion event.'
    )
    ...
    assert "activity_key=f'review:{run_id}:{document_id}'," in workflow_runner_content, (
        'Expected document review start and completion thoughts to reuse the same activity key.'
    )
    ...
    assert "if event_type == 'reduction_completed':" in route_content, (
        'Expected streamed document-action thoughts to label terminal review completion events.'
    )
