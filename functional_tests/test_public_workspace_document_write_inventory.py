# test_public_workspace_document_write_inventory.py
"""
Functional test for the public workspace document write inventory.
Version: 0.261.174
Implemented in: 0.261.174

A write that stores an outdated copy of a public workspace document can put back
membership, roles or settings that have since changed. A write to a workspace deleted
since it was read recreates it. So every writer of the public workspace document goes
through ``update_public_workspace_document_with_etag_guard`` in
``functions_public_workspaces``, apart from a short, reviewed allowlist.

This test reads every module of the application, statically, and finds every call to
a write method of ``cosmos_public_workspaces_container``: ``upsert_item``,
``replace_item``, ``create_item``, ``patch_item``, ``delete_item``,
``execute_item_batch`` and ``delete_all_items_by_partition_key``. It follows the
container:
...
- as ``<module>.cosmos_public_workspaces_container``, or through
  ``getattr(<object>, "cosmos_public_workspaces_container"[, default])``;
...
and calling a method this test doesn't know. The text
``"cosmos_public_workspaces_container"`` anywhere but ``config.py`` fails the same
way, since a registry can reach the container by name.
...
``PENDING_RAW_WRITES`` is the ratchet this slice uses while it moves the last
unconditional writers onto the guard, each conversion removing its own entries. A
pending entry that doesn't match the code fails, so it can only shrink: a new
unconditional writer has to go on the guard or be reviewed into the allowlist.
"""
...
CONTAINER = "cosmos_public_workspaces_container"
...
ALLOWED_WRITES = {
    ("functions_public_workspaces.py", "create_public_workspace", "create_item"):
        "creates a new public workspace document",
    ("functions_public_workspaces.py", "update_public_workspace_document_with_etag_guard", "replace_item"):
        "the guard itself: conditional on the _etag of the copy it just read",
    ("functions_public_workspaces.py", "delete_public_workspace", "delete_item"):
        "removes the whole public workspace",
    ("route_backend_control_center.py", "_execute_delete_public_workspace", "delete_item"):
        "removes the whole public workspace from the Control Center",
    ("functions_public_document_management.py", "_patch_tag_definitions", "patch_item"):
...
PENDING_RAW_WRITES = Counter({
    ...
    ("route_backend_retention_policy.py", "force_push_retention_defaults", "upsert_item"): 1,
    ("route_backend_retention_policy.py", "update_public_workspace_retention_settings", "upsert_item"): 1,
})
...
class Inventory:
    """Every use of the public workspaces container in one module's source."""
...
def test_the_inventory_reads_the_whole_application(inventories):
    ...
    assert found_writes(inventories)[
        ("functions_public_workspaces.py", "update_public_workspace_document_with_etag_guard", "replace_item")
    ] == 1


def test_every_public_workspace_write_is_allowed_or_pending(inventories):
    unexpected = {
        key: count for key, count in found_writes(inventories).items()
        if key not in ALLOWED_WRITES and key not in PENDING_RAW_WRITES
    }
    assert unexpected == {}, (
        "A new unconditional write of the public workspace document: "
        "use update_public_workspace_document_with_etag_guard"
    )
...
def test_the_tag_definition_patch_is_conditional_on_the_etag(inventories):
    [(_owner, _method, call)] = [
        write for write in inventories["functions_public_document_management.py"].writes
...
SYNTHETIC = {
    "direct": ("from config import cosmos_public_workspaces_container\n"
               "def f(g):\n    cosmos_public_workspaces_container.upsert_item(g)\n", [("f", "upsert_item")], []),
    "star_import": ("from config import *\n"
                    "def f(g):\n    cosmos_public_workspaces_container.replace_item(item=g['id'], body=g)\n",
                    [("f", "replace_item")], []),
    "import_alias": ("from config import cosmos_public_workspaces_container as pubs\n"
                     "def f(g):\n    pubs.create_item(g)\n", [("f", "create_item")], []),
    "alias_chain": ("from config import *\n"
                    "def f(g):\n    c = cosmos_public_workspaces_container\n    other = c\n    other.upsert_item(g)\n",
                    [("f", "upsert_item")], []),
    "closure": ("from config import *\n"
                "def f(g):\n    c = cosmos_public_workspaces_container\n    def apply():\n        c.patch_item(g, g, [])\n"
                "    apply()\n", [("apply", "patch_item")], []),
    "module_attribute": ("import config\n"
                         "def f(g):\n    config.cosmos_public_workspaces_container.delete_item(g, g)\n",
                         [("f", "delete_item")], []),
    "getattr": ("import config\n"
                "def f(g):\n    container = getattr(config, 'cosmos_public_workspaces_container', None)\n"
                "    container.upsert_item(g)\n", [("f", "upsert_item")], []),
    "positional_parameter": ("from config import cosmos_public_workspaces_container\n"
                             "def save(container, g):\n    container.upsert_item(g)\n"
                             "def f(g):\n    save(cosmos_public_workspaces_container, g)\n", [("save", "upsert_item")], []),
    "keyword_parameter": ("from config import cosmos_public_workspaces_container\n"
                          "def save(g, *, target=None):\n    target.execute_item_batch([])\n"
                          "def f(g):\n    save(g, target=cosmos_public_workspaces_container)\n",
                          [("save", "execute_item_batch")], []),
    "another_module": ("from config import cosmos_public_workspaces_container\nfrom helpers import save\n"
                       "def f(g):\n    save(cosmos_public_workspaces_container, g)\n", [], ["escapes into save"]),
    "returned": ("from config import cosmos_public_workspaces_container\n"
                 "def get():\n    return cosmos_public_workspaces_container\n", [], ["escapes into Return"]),
    "registry": ("from config import cosmos_public_workspaces_container\n"
                 "REGISTRY = {'public': cosmos_public_workspaces_container}\n", [], ["escapes into Dict"]),
    "attribute_store": ("from config import cosmos_public_workspaces_container\n"
                        "class Holder:\n    def __init__(self):\n        self.container = cosmos_public_workspaces_container\n",
                        [], ["escapes into Assign"]),
    "method_by_name": ("from config import cosmos_public_workspaces_container\n"
                       "def f(g):\n    getattr(cosmos_public_workspaces_container, 'upsert_item')(g)\n",
                       [], ["escapes into getattr"]),
    "unknown_method": ("from config import cosmos_public_workspaces_container\n"
                       "def f():\n    cosmos_public_workspaces_container.replace_throughput(400)\n",
                       [], ["uses .replace_throughput"]),
    "reads_only": ("from config import cosmos_public_workspaces_container\n"
                   "def f(g):\n    cosmos_public_workspaces_container.read_item(item=g, partition_key=g)\n"
                   "    list(cosmos_public_workspaces_container.query_items('SELECT * FROM c'))\n", [], []),
...
def test_the_registry_text_is_found_wherever_it_appears():
    inventory = Inventory("registry_text.py", "NAMES = ('cosmos_public_workspaces_container', 'other')\n")
