...
This test ensures exhaustive review and document comparison expose phase
...
def test_document_action_phase_progress_wiring():
    ...
    assert 'VERSION = "0.241.096"' in config_content, (
        "Expected config.py version 0.241.096 for phase-aware document action progress."
    )
    assert "'phase_label': progress_meta.get('phase_label')" in review_service_content, (
        "Expected exhaustive review progress snapshots to expose the current phase label."
    )
    assert "'phase_detail': progress_meta.get('phase_detail')" in review_service_content, (
        "Expected exhaustive review progress snapshots to expose phase detail text."
    )
    assert "'type': 'reduction_started'" in review_service_content, (
        "Expected exhaustive review activity callbacks to publish reduction-phase updates."
    )
    ...
    assert 'Combining review findings into the final response' in chat_route_content, (
        "Expected the chat route to describe the exhaustive review reduction phase."
    )
    ...
    assert 'Combining review findings into the final response' in workflow_runner_content, (
        "Expected workflow thoughts to describe the exhaustive review reduction phase."
    )
