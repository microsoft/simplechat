...
standard Semantic Ranker and Cosmos DB provisioned shared autoscale throughput,
...
def test_deployer_capacity_defaults() -> bool:
    ...
    assert_contains(bicep_cosmos, "autoscaleSettings", "Bicep Cosmos autoscale database throughput")
    ...
    assert_contains(azurecli_deployer, "--max-throughput $paramCosmosDbAutoscaleMaxThroughput", "Azure CLI Cosmos autoscale database create")
    ...
    assert_contains(terraform_main, 'resource "azurerm_cosmosdb_sql_database" "simplechat"', "Terraform Cosmos shared database")
    ...
    assert one_click_template["parameters"]["cosmosDatabaseAutoscaleMaxThroughput"]["defaultValue"] == 4000
    ...
    print("Free Search and serverless Cosmos DB are documented as explicit MVP/evaluation opt-ins.")
    return True
