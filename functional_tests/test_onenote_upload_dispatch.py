...
class OneNoteUploadDispatchTests(unittest.TestCase):
    ...
    def run_dispatch(self, suffix, scope, failure=None):
        ...
        namespace.update({
            ...
            "get_document_metadata": lambda *args, **kwargs: dict(metadata),
            "PUBLICATION_BINDING": "generated_artifact_publication_binding",
            "document_requires_screening": lambda document, settings: False,
        ...
        chunk_tests.load_functions(
            ...
            [
                ...
                "_resolve_processing_complete_status",
            ],
            namespace,
        )
    ...
    def test_missing_runtime_has_actionable_status_for_every_scope(self):
        for scope in ({}, {"group_id": "group-1"}, {"public_workspace_id": "public-1"}):
            with self.subTest(scope=scope):
                failure = onenote.OneNoteExtractionError("runtime_unavailable")
                _, metadata, source_exists = self.run_dispatch(".one", scope, failure)
                self.assertIn("Install the native component", metadata["status"])
                self.assertEqual(metadata["percentage_complete"], 0)
                self.assertFalse(source_exists)

    def test_screened_onenote_intake_keeps_the_existing_guard(self):
        for suffix in (".one", ".onepkg"):
            for scope in ({}, {"group_id": "group-1"}, {"public_workspace_id": "public-1"}):
                with self.subTest(suffix=suffix, scope=scope):
                    private_pipeline = Mock(return_value={"screening_status": "pending"})
                    processor = Mock()
                    namespace = {
                        "get_document_metadata": lambda *args: {"id": "document-1"},
                        "get_settings": lambda: {},
                        "PUBLICATION_BINDING": "generated_artifact_publication_binding",
                        "document_requires_screening": lambda document, settings: True,
                        "process_screened_upload": private_pipeline,
                    ...
                    result = namespace["process_document_upload_background"](
                        "document-1", "user-1", f"temporary{suffix}", f"fixture{suffix}", **scope,
                    )
                    self.assertEqual(result, {"screening_status": "pending"})
                    processor.assert_not_called()
                    private_pipeline.assert_called_once_with(
                        "document-1", "user-1", f"temporary{suffix}", f"fixture{suffix}", processor,
                        group_id=scope.get("group_id"), public_workspace_id=scope.get("public_workspace_id"),
