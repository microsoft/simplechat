...
For an approved self-hosted service or gateway, choose the OpenAI-compatible custom
connection type and use its actual API base path and API key/token. Enter model
names manually. Unknown models need explicit vector dimensions and input limits;
declaring support is not proof that the deployed service accepts the operation.
Custom connections in this release are embedding-only.
