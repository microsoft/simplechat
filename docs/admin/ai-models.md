...
- Choose authentication supported by the connection and grant required permissions: Azure discovery uses managed identity or a service principal; Custom connections support API keys, bearer tokens, or OAuth2 client credentials.
...
listing Azure deployments. Use **Test chat** or the separate image test to exercise
the intended inference operation; a successful chat test does not establish image
generation support. Image tests use a saved connection and may incur charges.
