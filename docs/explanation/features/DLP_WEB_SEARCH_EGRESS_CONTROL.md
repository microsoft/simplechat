...
- Presidio is not selectable in the admin UI until the service and embedded paths are wired into runtime execution and covered by tests. Presidio-style adapter normalization remains internal helper scaffolding only.
...
Regex DLP remains deterministic and dependency-light, but it is not equivalent to Presidio. Presidio remains the recommended future direction for contextual PII such as names, addresses, and natural-language identifiers.
...
Raw prompts, web-search queries, snippets, raw matched values, filenames, and Presidio explanations are excluded.
...
Regex DLP is intentionally lightweight. It is useful for structured identifiers such as SSNs, Luhn-valid credit card numbers, and administrator-defined exact-format identifiers, but it is weaker for names, addresses, contextual PII, international identifiers, secrets, and noisy prose. Presidio remains a future option, but it is not selectable in this release until runtime wiring and test coverage are added.
