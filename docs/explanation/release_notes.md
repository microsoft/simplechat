...
*   **First Batch Of Documentation Screenshots**
    *   Filled 51 empty screenshot slots, taking documentation screenshot coverage from 18 of 122 to 69 of 122. The Administration group is now fully illustrated.
    *   Added the four admin settings overviews (Backup & Recovery, Data Lifecycle, Governance, Workflow), six chat control references (conversation list, conversation header, composer, selectors, grounded search, and advanced conversation search), thirty task guide steps, and configuration panes for eleven action types (Azure Maps, Blob Storage, Chart, Cosmos Query, Databricks, Document Search, Log Analytics, MCP, Microsoft Graph, OpenAPI, RocksDB, and SimpleChat).
