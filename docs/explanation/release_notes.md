...
*   **Agent Instruction Context References (`#action` and `#knowledge`)**
    ...
    *   Tokens such as `#action:"Simple Chat":create_group` and `#knowledge:doc:"Employee Handbook.pdf"` are stored literally with the instructions so they stay editable and round-trip unchanged when an agent is edited.
    *   Navigate with the arrow keys, insert with `Tab` or `Enter`, dismiss with `Esc`, or use the mouse.
    *   (Ref: #1257, `agent_instruction_mentions.js`, `agent_modal_stepper.js`, `_agent_modal.html`)
...
*   **Context-Aware Draft Instructions**
    ...
    *   Client-supplied context is normalized, length-capped, and count-capped on the backend, and is used purely as prompt text — it never affects authorization.
    *   (Ref: #1257, `route_backend_agents.py`, `POST /api/agents/draft-instructions`)
...
*   **Agent Modal Step Reorder: Instructions After Actions and Knowledge**
    ...
    *   (Ref: #1257, `_agent_modal.html`, `agent_modal_stepper.js`)
