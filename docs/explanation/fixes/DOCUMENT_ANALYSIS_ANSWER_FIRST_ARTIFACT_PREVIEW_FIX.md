...
- Code changes summary: Added answer-summary synthesis for generated Analyze replies, including page/count table summarization for search-like requests, collapsed Analyze/Comparison artifact previews behind a native disclosure control, and removed inline mini-preview fragments from artifact-backed replies.
...
- Before/after comparison: Before the fix, artifact cards and previews were the dominant response surface. After the fix, the assistant message starts with an explicit answer summary, omits awkward inline table-preview fragments, and keeps artifact previews collapsed by default for Analyze/Comparison outputs.
