...
Tabular document actions launched from chat review and compare could sit on the initial queued badge for the full duration of the workbook run. The backend was still doing work, but the user did not see the existing tabular lifecycle or tool-progress thoughts, which made the request look stuck.
...
- The document-action stream callback in `route_backend_chats.py` only translated legacy review/comparison activity events and did not forward preformatted tabular thought payloads.
...
- Updated the document review and comparison workflow branches so tabular workbook analysis emits the same live progress thoughts as the main chat tabular path.
...
- Long-running tabular review and compare requests now show ongoing lifecycle and tool activity instead of a static queued badge.
- Users can tell the request is still active while workbook evidence is being gathered.
- The fix only changes progress/thought plumbing for tabular document actions; the underlying review and comparison logic is unchanged.

## Validation

- Before: tabular document actions could remain on `Queued exhaustive review...` or `Queued comparison...` with no visible progress during workbook analysis.
