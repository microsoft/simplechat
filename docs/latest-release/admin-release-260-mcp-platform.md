...
2. Open Admin Settings > Governance and apply destination or capability policies before making MCP actions broadly available.
3. Use Test Connection on configured MCP actions so users see reliable action availability.
4. Tell users which MCP clients or outbound destinations are approved before enabling the capability tenant-wide.

## Where to Find It

- **Open Agents** &mdash; Configure outbound MCP action presets and server-side catalogs.
- **Open Governance** &mdash; Control MCP destination and capability access.
